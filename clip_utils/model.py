...
import math
import numpy as np
from .attn2 import MultiheadAttention2
import torch
...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, self_qk_weight = False, self_out_weight = False):
        x_i, attn = self.attention(self.ln_1(x), self_qk_weight = self_qk_weight, self_out_weight = self_out_weight)
        x = x + x_i
        x = x + self.mlp(self.ln_2(x))
        return x, attn # 默认返回mean attn weights
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        attn_weights = []
        for idx,blk in enumerate(self.transformer.resblocks,start=1):
            if idx==11:
    ...
    def forward_origin(self, x: torch.Tensor):
        B, nc, w, h = x.shape
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
...
class CLIP(nn.Module):
    ...
    def encode_image_origin(self, image):
        return self.visual.forward_origin(image.type(self.dtype))
